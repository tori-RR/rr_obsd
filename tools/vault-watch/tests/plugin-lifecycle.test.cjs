...
/** Load the actual plugin entrypoint with an entirely in-memory vault. */
async function createHarness(t, { manifestDir, configDir = '.obsidian', stored = {} } = {}) {
  ...
  class Plugin {
    constructor(currentApp, manifest) { this.app = currentApp; this.manifest = manifest; this.commands = new Map(); this.tabs = []; }
    async loadData() { return { ...stored }; }
    async saveData(value) { saved.push(JSON.parse(JSON.stringify(value))); }
  ...
  class PluginSettingTab {
    constructor(currentApp) {
      this.app = currentApp;
      this.containerEl = { empty: () => controls.clear(), createEl: () => {} };
    }
  }
  class Setting {
  ...
  const dependencies = {
    obsidian: { Plugin, PluginSettingTab, Setting, Notice, normalizePath: (value) => value.replace(/\\/g, '/').normalize('NFC') },
    'node:path': path,
  ...
  vm.runInNewContext(source, {
    module,
    require(name) {
      assert.ok(Object.hasOwn(dependencies, name), `Unexpected entrypoint dependency: ${name}`);
      return dependencies[name];
    },
    process: { platform: 'win32' },
    Date,
  }, { filename: 'src/main.js' });
  const manifest = { id: 'vault-watch', ...(manifestDir ? { dir: manifestDir } : {}) };
  const plugin = new module.exports(app, manifest);
  t.after(async () => {
    plugin.onunload();
  ...
  return { root, plugin, app, adapter, disk, index, applied, bridges, notices, saved, controls,
    put, rename, launch, ready: () => ready(), blockNextAdapterCall };
}

test('actual entrypoint wires ready, file events, offline and reconnect into the real reconciliation engine', async (t) => {
...
test('settings apply immediately and the fallback helper path respects a custom config directory', async (t) => {
  ...
  assert.equal(original.options.helperPath, path.join(h.root, '.alternate-settings/plugins/vault-watch/native/watch.ps1'));
  h.plugin.tabs[0].display();
...
for (const action of ['pause', 'unload']) {
  test(`${action} cancels an apply already waiting in the host adapter queue and ignores obsolete bridge messages`, async (t) => {
    ...
    if (action === 'pause') await h.plugin.commands.get('toggle-watcher').callback();
