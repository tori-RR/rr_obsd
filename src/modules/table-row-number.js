'use strict';
// Table Row Number — 表格行号模块（自 tori 的 table-row-number 插件移植）。
// 样式注入、自动列宽、字体探测与设置项均保持原实现。
...
class TableRowNumberModule {
  ...
  addRibbonToggle() {
    this.ribbonIconEl = this.host.addRibbonIcon("list-ordered", "切换表格行号显示",
      () => this.toggleEnabled());
...
`;
  ...
  startAutoSize() {
    ...
    this.host.registerEvent(this.host.app.workspace.on("layout-change", debounced));
    this.host.registerEvent(this.host.app.workspace.on("active-leaf-change", debounced));
    this.host.registerEvent(this.host.app.workspace.on("resize", debounced));
  }
  ...
  reapply() {
    this.applyStyle();
    if (this.settings.enabled) this.resizeAll();
  }
  ...
  renderSettings(containerEl) {
    ...
    new Setting(containerEl)
      ...
      .setDesc("行号从几开始（默认 1）")
      .addText((t) =>
        t.setValue(String(settings.startFrom)).onChange(async (v) => {
          const n = parseInt(v, 10);
          if (!isNaN(n)) {
            settings.startFrom = n;
            await save();
            this.reapply();
          }
        })
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setValue(settings.separator).onChange(async (v) => {
    ...
    new Setting(containerEl)
      ...
      .addDropdown((d) => {
        ...
        d.setValue(settings.fontFamilyPreset)
          .onChange(async (v) => {
            settings.fontFamilyPreset = v;
            await save();
            this.reapply();
          });
      });
    ...
    new Setting(containerEl)
      ...
      .addText((t) =>
        t.setValue(settings.fontSize).onChange(async (v) => {
          settings.fontSize = v.trim() || "1em";
          await save();
          this.reapply();
        })
      );
    ...
    new Setting(containerEl)
      ...
      .addSlider((sl) => {
        sl.setLimits(100, 900, 100)
          .setValue(parseInt(settings.fontWeight, 10) || 600)
          .setDynamicTooltip()
          .onChange(async (v) => {
            settings.fontWeight = String(v);
            await save();
            this.reapply();
          });
      });
  ...
  buildCustomFontRow(containerEl) {
    const datalistId = "trn-fonts-datalist";
    // 提前建 datalist 元素（挂在 containerEl 下，全局可引用）
    let datalist = containerEl.querySelector(`#${datalistId}`);
    if (!datalist) {
      datalist = containerEl.createEl("datalist", { attr: { id: datalistId } });
    }
    ...
    setting.addText((t) => {
      t.setPlaceholder("输入字体名，如 Cascadia Code")
        .setValue(this.settings.customFontFamily)
        .onChange(async (v) => {
          this.settings.customFontFamily = v;
          await this.persist();
          this.reapply();
        });
...
module.exports = { TableRowNumberModule, TABLE_ROW_NUMBER_PRESETS: FONT_PRESETS };
